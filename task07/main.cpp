  ...
  for (unsigned int iw = 0; iw < img_width; ++iw) {
    for (unsigned int ih = 0; ih < img_height; ++ih) {
      ...
      for (int isample = 0; isample < nsample; ++isample) {
        ...
        float rad = hit0_brdf * std::max(0.f, hit0_normal.dot(hit0_refl)) * hit1_rad / (hit0_pdf * nsample);
      ...
      for (int isample = 0; isample < num_half_sample; ++isample) {
        ...
        float weight = std::clamp(hit0_pdf_brdf_sample / (hit0_pdf_brdf_sample + hit0_pdf_light_sample), 0.f, 1.f);
        float rad = weight * hit0_brdf * std::max(0.f, hit0_normal.dot(hit0_refl)) * hit1_rad / (hit0_pdf_brdf_sample * num_half_sample);
      ...
      for (int isample = 0; isample < nsample / 2; ++isample) {
        ...
        float hit0_pdf_light_sample = pdf_light_sample(hit0_normal, hit0_pos, cam_ray_dir, hit0_refl, hit0_object);
        float hit0_pdf_brdf_sample = spheres[hit0_object].pdf(hit0_normal, cam_ray_dir, hit0_refl);
        float weight = std::clamp(hit0_pdf_light_sample / (hit0_pdf_brdf_sample + hit0_pdf_light_sample), 0.f, 1.f);
        float rad = weight * hit0_brdf * std::max(0.f, hit0_normal.dot(hit0_refl)) * hit1_rad / (hit0_pdf_light_sample * num_half_sample);
        img_mis[ih * img_width + iw] += rad;
      }
    }
  }
